...
Бот устанавливается на VPS сервер ubuntu 20 и запускается в SCREEN (чтобы бот не отключался при разрыве SSH-соединения с VPS), настраивается telegram-бот и канал, куда приходит информация о работе бота. Запуск бота командой: ./ebotFuture-6, остановка бота командой: ctrl+c.
