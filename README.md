...
BinFuture выбирает монету, выросшую за выбранный в настройках период времени на указанный процент (при работе в  LONG, а при работе в SHORT упавшую), 
...
Рекомендуется BinFuture установить на VPS сервер ubuntu 20/22 и запускать в SCREEN (чтобы бот не отключался при разрыве SSH-соединения с VPS), настроить telegram-бот и канал, куда будет приходит информация о работе бота.

При необходимости E-Bot можно использовать и на Windows, для этого нужно скачать в отдельную папку версию для Windows (https://github.com/ebot732/ebotFuture/releases/download/ebotFuture-9.2/ebotFuture-9.2.exe), и запустить.
...
Для работы BinFuture можно использовать BNB для оплаты комиссий биржи (нужно перевести нужное количество BNB на фьючерсный счет в лк binance) и следить за наличием BNB на Futures аккаунте.

После закрытия каждой сделки BinFuture:
...
- min_bal_perc: минимальный процент от депозита, ниже которого BinFuture не будет выставлять усредняющий ордер, 
...
Здесь BinFuture представлен для ознакомления и использования в течении пробного периода до 15 мая 2023 г.
Если Вы хотите увеличить время работы до 1/6/12 месяцев: напишите в телеграм, по данным, указанным при запуске E-Bot.

Если хотите испытать BinFuture на фьючерсной тестовой бирже Binance- 
переходите на:
...
Редко, но бывает, что сервера telegram кратковременно недоступны, и в этот момент сообщение от BinFuture может не доходить в канал бота. 
...
Установка и запуск BinFuture:
- на VPS-сервере ubuntu 20 создайте новую папку, например, BinFuture (mkdir BinFuture)
- зайдите в эту папку (cd BinFuture)
- перенесите в эту папку файл бота BinFuture-12 (или скачайте с github командой: wget https://github.com/ebot732/ebotFuture/releases/download/ebotFuture-12/BinFuture-12)
- откройте screen-сессию (например: screen -S BinFuture)
- дайте права запуска файлу (команда: chmod 755 BinFuture-12)
- запустите E-Bot (команда: ./BinFuture-12)
...
- для входа в screen работающего бота используйте команду: screen -x BinFuture

Для удобства настройки BinFuture используется телеграм бот, которого нужно сделать админом в телеграм канале. Необходимые данные телеграм бот возьмет из БД BinFuture и будет управляться через чат telegram-Botа.
...
![Screenshot](https://github.com/ebot732/BinFuture/blob/main/screenshots/Screenshot_20221124-185022_Telegram.jpg)

====================================================================================================================================

![Screenshot](https://github.com/ebot732/BinFuture/blob/main/screenshots/Screenshot_20230308-120835.png)

====================================================================================================================================

![Screenshot](https://github.com/ebot732/BinFuture/blob/main/screenshots/Screenshot_20230308-123701.png)

====================================================================================================================================

![Screenshot](https://github.com/ebot732/BinFuture/blob/main/screenshots/%D0%A1%D0%BD%D0%B8%D0%BC%D0%BE%D0%BA%20%D1%8D%D0%BA%D1%80%D0%B0%D0%BD%D0%B0%20%D0%BE%D1%82%202023-03-05%2008-16-08.png)

====================================================================================================================================

![Screenshot](https://github.com/ebot732/BinFuture/blob/main/screenshots/%D0%A1%D0%BD%D0%B8%D0%BC%D0%BE%D0%BA%20%D1%8D%D0%BA%D1%80%D0%B0%D0%BD%D0%B0%20%D0%BE%D1%82%202023-03-08%2012-13-27.png)
